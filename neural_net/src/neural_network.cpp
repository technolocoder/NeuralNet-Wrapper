...
TEMPLATE void neural_network<T>::construct_neuralnet(){
    ...
    if(fl_index>0){
        ...
        int type = layers[fl_index-1].type;
...
TEMPLATE T* neural_network<T>::feedforward(T *input){
    ...
    return layers[layer_count-1].type==POOLING_LAYER?layers[layer_count-1].PL.o_ptr:layers[layer_count-1].type==CONVOLUTION_LAYER?layers[layer_count-1].CL.o_ptr:layers[layer_count-1].type==FLATTEN_LAYER?layers[layer_count-1].FL.ptr:layers[layer_count-1].IL.o_ptr;
}
...
TEMPLATE dim neural_network<T>::get_output_dimensions(){
    if(layers[layer_count-1].type == POOLING_LAYER){
        ...
        return {layers[layer_count-1].FL.total,1,1};
    }
